...
int nec_rx_init(PIO pio, uint pin_num) {
  ...
  if (pio_can_add_program(pio, &nec_receive_program)) {
    int offset_result = pio_add_program(pio, &nec_receive_program);
    if (offset_result < 0) {
      return -1;
    }
    offset = (uint)offset_result;
  } else {
  ...
  if (sm_result == -1) {
    pio_remove_program(pio, &nec_receive_program, offset);
    return -1; // we were unable to claim a state machine
  }
  uint sm = (uint)sm_result;
